...
class GcsPathParserSpec extends AnyFlatSpecLike with Matchers with EitherValues {
  ...
  "gcs" should "generate valid bucket names" in {
    ...
    generateUniqueBucketName("myClusterWhichHasAVeryLongNameBecauseIAmExtremelyVerboseInMyDescriptions").value should startWith ("myclusterwhichhasaverylong-")
  ...
  "GcsPath" should "parse valid paths" in {
    ...
    parseGcsPath("gs://a-bucket-123").right.value shouldBe GcsPath(GcsBucketName("a-bucket-123"), GcsObjectName(""))
    parseGcsPath("//a-bucket-123/a/relative/path").right.value shouldBe GcsPath(GcsBucketName("a-bucket-123"), GcsObjectName("a/relative/path"))
    parseGcsPath("gs://a_123_bucket/a/relative/path").right.value shouldBe GcsPath(GcsBucketName("a_123_bucket"), GcsObjectName("a/relative/path"))
    parseGcsPath("gs://a.123.bucket.123/a/relative/path").right.value shouldBe GcsPath(GcsBucketName("a.123.bucket.123"), GcsObjectName("a/relative/path"))
    parseGcsPath("gs://a-bucket_123.456/a/relative/path").right.value shouldBe GcsPath(GcsBucketName("a-bucket_123.456"), GcsObjectName("a/relative/path"))
    parseGcsPath("gs://a-bucket-with-a-slightly-longer-name-42/a/relative/path").right.value shouldBe GcsPath(GcsBucketName("a-bucket-with-a-slightly-longer-name-42"), GcsObjectName("a/relative/path"))
  }
  ...
  it should "fail to parse invalid paths" in {
    ...
    parseGcsPath("gs://a-bucket-which-has-a-very-long-name-because-i-am-extremely-verbose-in-my-descriptions-but-is-otherwise-valid/foo").left.value shouldBe a [GcsParseError]
  }

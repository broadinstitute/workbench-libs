...
class KubernetesInterpreter[F[_]: StructuredLogger: Effect: Timer: ContextShift](
  ...
  private def getClient[A](clusterId: KubernetesClusterId, fa: ApiClient => A): F[A] =
    for {
      client <- F.delay(cache.get(clusterId))
      token <- getToken()
      _ <- F.delay(client.setApiKey(token.getTokenValue))
    } yield fa(client)

...
trait GoogleKubernetesService[F[_]] {
  def createCluster(kubernetesClusterRequest: KubernetesCreateClusterRequest): F[KubernetesClusterIdentifier]
...
object Test {
  ...
  def callCreateCluster() = createCluster(KubernetesCreateClusterRequest(parent, clusterName, nodePoolName, None))
  ...
  def testKubeClient() = serviceResource.use {
    service => {
      for {
        _ <- service.initAuthToCluster(KubernetesClusterIdentifier(project, location, clusterName))
        _ <- service.asInstanceOf[GoogleKubernetesInterpreter[IO]].testClientInteraction()
      } yield ()
    }
  }
}
...
final case class KubernetesCreateClusterRequest(parent: Parent, clusterName: KubernetesClusterName, initialNodePoolName: NodePoolName, clusterOpts: Option[KubernetesClusterOpts])

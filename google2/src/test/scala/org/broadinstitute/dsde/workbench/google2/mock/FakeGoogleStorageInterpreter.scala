...
class BaseFakeGoogleStorage extends GoogleStorageService[IO] {
  ...
  override def insertBucket(bucketName: GcsBucketName, googleProject: Option[GoogleProject] = None, acl: Option[NonEmptyList[Acl]] = None, labels: Map[String, String] = Map.empty, traceId: Option[TraceId] = None): Stream[IO, Unit] = Stream.empty

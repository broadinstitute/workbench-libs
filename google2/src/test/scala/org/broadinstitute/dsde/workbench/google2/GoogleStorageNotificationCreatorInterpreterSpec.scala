  ...
  "notificationResponseDecoder" should "decode NotificationResopnse properly" in {
    forAll {
      (notificationResponse: NotificationResponse) =>
        val jsonString = notificationResponse.items.map{
          notifications =>
  ...
  "notificationRequestEnoder" should "encode NotificationRequest properly" in {
    forAll {
      (notificationRequest: NotificationRequest) =>
        val eventTypesString = if(notificationRequest.eventTypes.isEmpty)
          ""
        else
          notificationRequest.eventTypes.map(_.asString).mkString("\"", "\",\"", "\"")
        val eventTypes = s""" "event_types": [$eventTypesString]"""
        val objectNamePrefix = s""" "object_name_prefix": ${notificationRequest.objectNamePrefix.map(s => "\""+s+"\"").getOrElse("null")} """
        ...
        val expectedJson = parse(expectedJsonString).getOrElse(throw new Exception("failed to parse NotificationRequest json"))

...
object GoogleFirestoreInterpreters {
      ...
      IO.async[A] { cb =>
        ApiFutures.addCallback(
          db.runTransaction(new Transaction.Function[A] {
            override def updateCallback(transaction: Transaction): A =
              ops(db, transaction).unsafeRunSync()
          }),
          callBack(cb),
          executor
        )
  ...
  def futureFirestore(db: Firestore)(
    ...
    override def set(
        collectionName: CollectionName,
        document: Document,
        dataMap: Map[String, Any]): Future[Instant] =
      iofs.set(collectionName, document, dataMap).unsafeToFuture()
    override def get(collectionName: CollectionName, document: Document): Future[DocumentSnapshot] =
      iofs.get(collectionName, document).unsafeToFuture()
    override def transaction[A](
        ops: (Firestore, Transaction) => Future[A]
    ): Future[A] =
      iofs
        .transaction((fs, tx) => IO.fromFuture(IO(ops(fs, tx))))
        .unsafeToFuture()
  }

...
private [azure] class AzurePublisherInterpreter[F[_]: Async: StructuredLogger](
                                                                                clientWrapper: AzureServiceBusSenderClientWrapper) extends AzurePublisher[F] {
  ...
  override def publishNative: Pipe[F, ServiceBusMessage, Unit] = in => in.flatMap(s => Stream.eval(publishServiceBusMessage(s)))
  ...
  private def publishServiceBusMessage(message: ServiceBusMessage): F[Unit] = {
    withLogging(
      ...
      clientWrapper
        ...
        .subscribe()
    }, Option(message.getCorrelationId).map(s => TraceId(s)), s"Publishing message to Service Bus", actionName = "azureServiceBusCall")
  }
}


object AzurePublisherInterpreter {
  def publisher[F[_]: Async: StructuredLogger](clientWrapper: AzureServiceBusSenderClientWrapper): Resource[F, AzurePublisher[F]] =
    for {

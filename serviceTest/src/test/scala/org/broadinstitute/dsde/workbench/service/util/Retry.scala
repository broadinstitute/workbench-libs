...
object Retry extends LazyLogging {
  ...
  /**
   * Retry an operation periodically until it returns something or no more retries remain. Returns when the operation
   * returns Some or all retries have been exhausted.
   *
   * @param remainingBackOffIntervals
   *   wait intervals between retries
   * @param op
   *   operation to retry
   * @return

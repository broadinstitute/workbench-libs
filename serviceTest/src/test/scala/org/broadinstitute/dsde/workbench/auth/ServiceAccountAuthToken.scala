...
case class ServiceAccountAuthTokenFromJson(privateKeyJsonString: String, scopes: Option[Seq[String]] = None)
  ...
  override def buildCredential(): GoogleCredential =
    GoogleCredential
      .fromStream(new ByteArrayInputStream(privateKeyJsonString.getBytes()))
      .createScoped(scopes.getOrElse(AuthTokenScopes.serviceAccountScopes).asJava)
}

object ServiceAccountAuthTokenFromPem {
  def apply(clientId: String, pemFilePath: String, scopes: Option[Seq[String]] = None) =
    new ServiceAccountAuthTokenFromPem(clientId, pemFilePath, scopes)
}
